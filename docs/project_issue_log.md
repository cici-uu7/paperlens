...
- Decision: 不再只保留通用的“改动 -> 验证 -> 记录 -> 提交 -> 推送”闭环说明，而是把用户明确要求的沟通方式、能力边界、联查范围、验证要求、文档同步和禁止事项都写入仓库根目录 `AGENTS.md`。
- Why: PaperLens 同时涉及回答链路、引用展示、评测产物、启动脚本和对外展示口径；如果 `AGENTS.md` 只写通用 git 收尾规则，后续 agent 仍可能在“只看单脚本、凭印象下结论、漏同步 README / reports / 日志”这些点上反复踩坑。
- Impact: 后续在这个仓库工作的 agent，除了要走闭环收尾，还应默认遵守“先读代码和文档、默认中文、修改时联查相关链路、不要把未完成能力写成已完成”等项目特定要求。

### [decision][P2][accepted] 仓库新增 `AGENTS.md` 固化“改动 -> 验证 -> 记录 -> 提交 -> 推送”闭环
- Decision: 在仓库根目录新增 `AGENTS.md`，明确后续 agent 处理真实改动任务时，默认必须按“实现、验证、写日志、必要时更新 `.autonomous`、提交、推送”的顺序收尾。
- Why: 本轮已经暴露出“代码改完了，但日志和 git 收尾没有在同一轮继续做完”的流程断层；把这条规则写进仓库，比仅靠当次口头约定更稳定。
- Impact: 后续同类任务除非用户明确要求停在分析、只改不提、或不推送，否则都应走完整闭环，不再停留在半完成状态。
